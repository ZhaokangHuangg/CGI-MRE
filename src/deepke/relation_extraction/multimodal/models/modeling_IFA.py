...
class IFAEncoder(nn.Module):
    ...
    def forward(
        ...
        # ture 1,0,0
        # hzk add vision model
        return BaseModelOutput(
            last_hidden_state=text_hidden_states, hidden_states=all_text_hidden_states, attentions=all_text_attentions
        ), BaseModelOutput(
            last_hidden_state=vision_hidden_states, hidden_states=all_text_hidden_states, attentions=all_text_attentions
        )
...
class IFAModel(nn.Module):
    def __init__(self, vision_config, text_config, config, add_pooling_layer=True):
    ...
    def forward(
        ...
        # bert_output try
        # 32,61,768
        # bert_output=text_embedding_output
        # # 32,61,768
        # masked_output = torch.mul(attention_mask.unsqueeze(2), bert_output)
        # # 32,1
        ...
        #
        # vision_embedding_output = torch.sum(masked_output, dim=1, keepdim=False)
        # utterance_text = vision_embedding_output


        # o=self.HZK(vision_embedding_output=vision_embedding_output,text_embedding_output=utterance_text)
        # return o

        # deepke zs
        # all encoder

        encoder_outputs,hzk_v_outputs = self.encoder(
            # (31.61.768)
        ...
        # hzk add
        hzk_v_output = hzk_v_outputs[0]
        pooled_output_v = self.text_pooler(hzk_v_output)
        ...
        return BaseModelOutputWithPooling(
            ...
            last_hidden_state=hzk_v_output,
            pooler_output=pooled_output_v,
            hidden_states=hzk_v_outputs.hidden_states,
            attentions=hzk_v_outputs.attentions,
        )
