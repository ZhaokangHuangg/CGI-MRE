        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if p.dtype == torch.float16:
                    ...
                    device_to_group[p_device]['v_16'].append(state['exp_avg_sq'])
                    ...
                    device_to_group[p_device]['v_bf'].append(state['exp_avg_sq'])
                    ...
                    device_to_group[p_device]['v_32'].append(state['exp_avg_sq'])
                else:
                    raise RuntimeError('FusedAdam only support fp16 and fp32.')

            for device in device_to_group:
                g_16, p_16, m_16, v_16 = device_to_group[device]['g_16'], device_to_group[device]['p_16'], device_to_group[device]['m_16'], device_to_group[device]['v_16']
                g_bf, p_bf, m_bf, v_bf = device_to_group[device]['g_bf'], device_to_group[device]['p_bf'], device_to_group[device]['m_bf'], device_to_group[device]['v_bf']
                g_32, p_32, m_32, v_32 = device_to_group[device]['g_32'], device_to_group[device]['p_32'], device_to_group[device]['m_32'], device_to_group[device]['v_32']
