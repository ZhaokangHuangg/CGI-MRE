"""
    Autoregressive Structured Prediction T5 model for Entity Relation Extraction
    Tianyu Liu
"""
import sys
import os
...
class T5ERE(T5PreTrainedModel):
    ...
    def __init__(
        ...
        self.num_linking_classes = num_linking_classes
        ...
        self.rr_scorer = util.make_ffnn(
            ...
            hidden_size=[asp_hidden_dim],
            output_size=1+2*self.num_linking_classes,
            dropout=self.dropout,
            std=self.init_std,
            activation=asp_activation
        )
    ...
    def forward(
        ...
        Inputs:
            ...
            decoder_linking (`List[torch.LongTensor]` each element is shape `(batch_size, number_of_r)`, *optional*, *inference*):
                A list of tensors. Each tensor is a tensor of shape `(batch_size, number_of_r)`, decoded linking actions until now.
        ...
        Returns:
            output (`MySeq2SeqLMOutput` ):
                ...
                linking (`torch.LongTensor` of shape `(batch_size, number_of_r)`, linking actions of this step, *inferece* only)
        ...
        if labels is not None:  # Training: We compute loss
            ...
            denom = logsumexp(torch.cat([
                    action_logits,
                    denom
                ], dim=-1), dim=-1)
            numer = logsumexp(torch.cat([
            ...
            if same_sentence_flag is not None:
                loss = loss / target_sentence_idx.max().item()
            l_choice, r_choice, typing_choice = None, None, None
            ...
            # r_choice: (batch_size, number_of_r)
            (denom, l_choice, typing_choice, r_choice) = self.get_logits_inference(
                outputs.last_hidden_state,  # current_hidden_state
    ...
    def get_logits_inference(
        ...
        if is_l.sum() == 0:
            ...
            lr_pair_emb = prepare_pair_embeddings(
            ...
            for i in range(batch_size):
                if num_l_each_instance[i]-self.max_nest_depth > 0:
            ...
            if is_r.sum() > 0:
                ...
                # (batch_size, 1, num_kept_r, num_linking_classes) / (batch_size, 1, num_kept_r)
                rr_score = self.get_rr_relation_score(
                    r_emb, current_hidden_state, distance_to_previous_r
                )
                # (batch_size, 1, num_r, 1+num_linking_classes)
                rr_score[..., 1:] += (~r_emb_mask * NEGINF).unsqueeze(1).unsqueeze(-1)
                # (batch_size, num_r)
                r_choice = (rr_score.argmax(dim=-1) - 1).squeeze(1)

        return (denom, l_choice, typing_choice, r_choice)
    ...
    def get_logits_training(
        ...
        decoder_output = self.dropout(decoder_output) # one dropout for all
        ...
        kept_is_after_l = is_after_l.gather(dim=2, index=prev_l_indices).unsqueeze(-1)

        lr_score = self.lr_scorer(lr_pair_emb) + (~kept_is_after_l) * NEGINF
    ...
    def get_rr_relation_score(
        ...
        seq_len, num_r = dec_emb.size(1), r_emb.size(1)
        pair_emb_1 = torch.cat([
            r_emb.unsqueeze(1).expand(-1, seq_len, -1, -1),
            dec_emb.unsqueeze(2).expand(-1, -1, num_r, -1)
        ], dim=-1)
        # pair_emb_2 = torch.cat([
        #     dec_emb.unsqueeze(2).expand(-1, -1, num_r, -1),
        #     r_emb.unsqueeze(1).expand(-1, seq_len, -1, -1),
        # ], dim=-1)

        if inv_relation:
            rr_score = self.rr_scorer(pair_emb_1)
            return rr_score

        # (batch_size, seq_len, max_num_r, 2, num_linking_classes)
        rr_score = self.rr_scorer(
            torch.stack([pair_emb_1, pair_emb_2], dim=-2)
        )
        # (batch_size, seq_len, max_num_r, 2*num_linking_classes)
        rr_score = rr_score.view(*pair_emb_1.size()[:-1], -1)
        return rr_score
